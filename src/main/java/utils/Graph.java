...
public class Graph implements Serializable {
    ...
    public String getPrice(String label) {
        GraphEdge edge = hasEdge(label);
        if(edge != null)
            return ((Trades) (edge.getValue())).getLastPrice();
        else
            return "NA";
    }
    ...
    public void DFSPath(String origin, String goal, List<Path> pathList) {
        ...
        if (this.vertices.isEmpty())
            ...
            if (!currentVertex.equals(destination))
                dfsPathRecursive(currentVertex, destination, assetList, pathList, priceList,lastPrice);
        }
    ...
    private void dfsPathRecursive(GraphVertex currentVertex,GraphVertex destination,
            ...
            List<Path> pathList, List<String> priceList, String lastPrice) {
        ...
        if(currentVertex.equals(destination)){
            ...
            //iterate through the adjacent vertices of the current vertex - recursively if necessary
            for (GraphVertex adjVertex : currentVertex.getAdjacent() ) {
                ...
                if(!adjVertex.getVisited()){
                    ...
                    //retrieve the price
                    lastPrice = ((Trades) edge.getValue()).getLastPrice();
                    // insert edge price data price path
                    priceList.add(lastPrice);
                    // recursive call to access the links of each asset
                    dfsPathRecursive(adjVertex, destination,assetList, pathList, priceList,lastPrice);
                    // delete vertex from path
            ...
            for (GraphVertex vertex : this.vertices) {
                ...
                for (GraphVertex adjacentVertex : adjacentVertices) {
                    ...
                    try {
                        ...
                        price = Double.parseDouble(((Trades) edge.getValue()).getLastPrice());
                    } catch (Exception e) {}
